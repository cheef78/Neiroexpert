class RailWay: #Класс Железная дорога. Содержит в себе всю необходимую информацию для расчета повреждаемости и прогнозирования
    ...
    def __init__(self, Critical_values, Rail_plan, Rail_profile, VSP, Roadbed, Climate): #определяем необходимые атрибуты класса - это план, профиль, верхнее строение пути, зем.полотно, климат, общие эксплуатационные характеристики, характеристики вагонопотока, поездопотока
        ...
        self.Critical_values = Critical_values #атрибут содержит в себе общие праивла эксплуатации (допустимые и критические отказы)
        ...
        self.Rail_resource = #TBI - атрибут содержит в себе текущий запас прочности рельс. По умолчанию он либо нормализован до 1, либо находится из входных данных

        self.VSP_resource = #TBI - атрибут содержит в себе текущий запас прочности элементов ВСП. По умолчанию он либо нормализован до 1, либо находится из входных данных

        self.Roadbed_resource = #TBI - атрибут содержит в себе текущий запас прочности элементов земного полотна. По умолчанию он либо нормализован до 1, либо находится из входных данных
    ...
    def Critical_values_info(self):
        print (str(self.Critical_value) + "-это допустимые параметры эксплутатации")
...
class Train: # Класс Поезд. Состоит из нескольких вагонов. 
    ...
    def add_wagon(self, count, wagon):
        self.wagons.extend([wagon for i in range(count))
